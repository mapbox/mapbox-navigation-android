...
class MapboxSpeechApiTest {
    ...
    fun `generatePredownloaded for first instruction gets cancelled by another generatePredownloaded for first instruction`() =
        coroutineRule.runBlockingTest {
            ...
            sut.predownload(listOf(instruction1))

            coVerify(exactly = 1) { consumer1.accept(match { it.isError }) }
        }

    @Test
    @Suppress("MaxLineLength")
    fun `generatePredownloaded for first instruction gets cancelled by another generatePredownloaded for non-first instruction`() =
        coroutineRule.runBlockingTest {
            val consumer1: MapboxNavigationConsumer<Expected<SpeechError, SpeechValue>> =
                mockk(relaxed = true)
            val consumer2: MapboxNavigationConsumer<Expected<SpeechError, SpeechValue>> =
            ...
            val instruction2 = VoiceInstructions.builder().announcement("dance and jump").build()
            every { firstInstructionsChecker.isFirstVoiceInstruction(instruction1) } returns true
            every { firstInstructionsChecker.isFirstVoiceInstruction(instruction2) } returns false
