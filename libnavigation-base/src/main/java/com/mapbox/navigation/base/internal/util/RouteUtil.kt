package com.mapbox.navigation.base.internal.util
...
class RouteUtil {
    ...
    fun calculateRemainingWaypointIndices(routeProgress: RouteProgress): IntArray? {
        ...
        return try {
            ...
            val resultWaypointIndices = IntArray(remainingWaypointIndices.size + ORIGIN_WAYPOINT_NAME_THRESHOLD)
            resultWaypointIndices[ORIGIN_WAYPOINT_INDEX] = allWaypointIndices[ORIGIN_WAYPOINT_INDEX].toInt()
            for (i in remainingWaypointIndices.indices) {
                resultWaypointIndices[i + 1] = remainingWaypointIndices[i].toInt() - traveledCoordinatesCount
            }
