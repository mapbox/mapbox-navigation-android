...
class MetricsRouteProgress(routeProgress: RouteProgress?) {
    ...
    private fun obtainRouteData(route: Route) {
    ...
    private fun obtainStepData(routeProgress: RouteProgress) {
        ...
        legProgress?.upComingStep()?.let { upcomingStep ->
            ...
            upcomingStepInstruction = upcomingStep.stepManeuver().instruction()
            upcomingStepType = upcomingStep.stepManeuver().type()
            upcomingStepModifier = upcomingStep.stepManeuver().modifier()
        }
        legProgress?.currentStep()?.let { currentStep ->
            previousStepInstruction = currentStep.stepManeuver().instruction()
            previousStepType = currentStep.stepManeuver().type()
            previousStepModifier = currentStep.stepManeuver().modifier()
        }
        previousStepName = currentStepName
    }

    private fun retrieveRouteDestination(route: Route): Point =
        route.legs()?.lastOrNull()?.steps()?.lastOrNull()?.stepManeuver()?.location()
            ?: DEFAULT_POINT
}
