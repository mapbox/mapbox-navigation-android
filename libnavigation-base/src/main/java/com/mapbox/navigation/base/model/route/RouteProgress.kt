...
import com.mapbox.navigation.base.model.voice.VoiceInstruction
...
data class RouteProgress internal constructor(
     ...
     * @since 0.1.0
     */
    fun currentLeg(): RouteLeg? = ifNonNull(route()?.legs(), legIndex) { legs, legIndex ->
        legs[legIndex]
    }
    ...
    fun distanceTraveled(): Double? =
        ifNonNull(route?.distance(), distanceRemaining) { distance, distanceRemaining ->
    ...
    fun fractionTraveled(): Float? =
        ifNonNull(distanceTraveled(), route()?.distance()) { distanceTraveled, distance ->
            when (distance > 0) {
                true -> {
    ...
    fun remainingWaypoints(): Int? =
        ifNonNull(route?.legs(), legIndex) { legs, legIndex -> legs.size - legIndex }
    ...
    class Builder {
        ...
        fun build(): RouteProgress {
            ...
            val leg: RouteLeg? = route?.let { directionRoute ->
