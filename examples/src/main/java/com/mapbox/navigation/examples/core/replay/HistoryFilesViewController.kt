...
class HistoryFilesViewController(
    ...
    private fun requestHistory(context: Context, connectionCallback: (Boolean) -> Unit): Job {
        return CoroutineScope(Dispatchers.Main).launch {
            ...
            drives.addAll(requestHistoryReplayDirectory(context))
    ...
    private suspend fun requestHistoryDisk(
        ...
        historyFiles.filter { it.endsWith(".json") || it.endsWith(".pbf") }
            .map { fileName ->
                ReplayPath(
    ...
    private suspend fun requestHistoryReplayDirectory(
        context: Context
    ): List<ReplayPath> = withContext(Dispatchers.IO) {
        historyFilesRepository.replayFiles(context)
            .map { file ->
                ReplayPath(
                    title = context.getString(R.string.history_replay_directory),
