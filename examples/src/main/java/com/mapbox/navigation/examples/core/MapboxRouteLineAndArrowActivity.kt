...
class MapboxRouteLineAndArrowActivity : AppCompatActivity(), OnMapLongClickListener {
            ...
            if (
                ...
                routeLineApi.findClosestRoute(
                    ...
                    object : MapboxNavigationConsumer<Expected<ClosestRouteValue, RouteNotFound>> {
                        override fun accept(result: Expected<ClosestRouteValue, RouteNotFound>) {
                            if (
                                result is Expected.Success &&
                                result.value.route != routeLineApi.getPrimaryRoute()
                            ) {
                                routeLineApi.updateToPrimaryRoute(
                                    result.value.route,
                                    updateToPrimaryRouteConsumer
                                )
                            }
                        }
                    }
    ...
    private val updateToPrimaryRouteConsumer =
        object : MapboxNavigationConsumer<Expected<RouteSetValue, RouteLineError>> {
            override fun accept(value: Expected<RouteSetValue, RouteLineError>) {
                // RouteLine: The above call updates the state of the
                // MapboxRouteLineApi but doesn't update the map until
                // the result of the call is rendered by the
                // MapboxRouteLineView.
                // However, in this example the rendering is not happening
                // here because the necessary call below to update
                // MapboxNavigation will result in a call to RoutesObservers
                // which is being used above. That observer will render
                // the line(s) on the map so rendering the result from the
                // call above would result in redundant renderings. If the
                // RoutesObserver was not being used and/or the observer wasn't
                // rendering the route line(s) then the result from the call
                // above would need to be rendered in order to update the map.
