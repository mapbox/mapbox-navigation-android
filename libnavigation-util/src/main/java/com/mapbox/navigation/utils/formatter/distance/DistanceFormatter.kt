...
import com.mapbox.navigation.base.model.route.MetricsRouteProgress
...
class DistanceFormatter(
    context: Context,
    language: String?,
    @VoiceUnitCriteria unitType: String,
    @param:RoundingIncrement @field:RoundingIncrement
private val roundingIncrement: Int
    ...
    init {
        ...
        this.unitType =
            if (IMPERIAL != unitType && METRIC != unitType) {
                context.inferDeviceLocale().getUnitTypeForLocale()
            } else {
                unitType
            }

        largeUnit = if (IMPERIAL == unitType) UNIT_MILES else UNIT_KILOMETERS
        smallUnit = if (IMPERIAL == unitType) UNIT_FEET else UNIT_METERS
    }
