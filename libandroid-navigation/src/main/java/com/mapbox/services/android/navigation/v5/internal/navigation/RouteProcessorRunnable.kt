...
internal class RouteProcessorRunnable(
    ...
    private val ARRIVAL_ZONE_RADIUS = 40
    ...
    private fun process() {
        ...
        routeProcessor.buildNewRouteProgress(mapboxNavigator, status, route)?.let { routeProgress ->
            ...
            val userOffRoute = isUserOffRoute(options, status, rawLocation, routeProgress, engineFactory)
            val snappedLocation = findSnappedLocation(status, rawLocation, routeProgress, engineFactory)
            val checkFasterRoute = checkFasterRoute(options, snappedLocation, routeProgress, engineFactory,
                    userOffRoute)
    ...
    private fun checkForNewLegIndex(
        ...
        val routeLegsSize = route.legs()?.let { legs ->
            if (legs.isEmpty())
                0
            else
                legs.size - 1
        ...
        val canUpdateLeg = previousState == RouteState.COMPLETE && previousLegIndex < routeLegsSize
    ...
    private fun findTriggeredMilestones(mapboxNavigation: MapboxNavigation, routeProgress: RouteProgress?): List<Milestone> {
