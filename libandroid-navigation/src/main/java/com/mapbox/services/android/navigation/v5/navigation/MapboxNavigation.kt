...
import com.mapbox.services.android.navigation.v5.navigation.NavigationConstants.NON_NULL_APPLICATION_CONTEXT_REQUIRED
...
class MapboxNavigation : ServiceConnection {
    // TODO public?
    var eventDispatcher = NavigationEventDispatcher()
        private set
    private var navigationEngineFactory = NavigationEngineFactory()
    private lateinit var navigationTelemetry: NavigationTelemetry
    ...
    private var _locationEngine: LocationEngine
        set(value) {
            ...
            if (isServiceAvailable) {
                navigationService.updateLocationEngine(value)
            }
        }
    ...
    private var routeRefresher: RouteRefresher? = null
    ...
    constructor(context: Context, accessToken: String,
        ...
        this.accessToken = accessToken
        this.options = options
        this._locationEngine = _locationEngine
        initialize()
    }
...
// Package private (no modifier) for testing purposes
    constructor(context: Context, accessToken: String,
        ...
        this._locationEngine = _locationEngine ?: LocationEngineProvider.getBestLocationEngine(context)
    ...
    private fun stopNavigationService() {
        ...
        if (isServiceAvailable && isBound.compareAndSet(true, false)) {
    ...
    override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
        ...
        navigationService = (service as LocalBinder).getService()
    ...
    private fun initializeForTest(context: Context) { // Initialize event dispatcher and add internal listeners
        _locationEngine = obtainLocationEngine()
        ...
        val offlineNavigator = OfflineNavigator(mapboxNavigator.navigator,
                ...
                accessToken)
        val locationEngine = _locationEngine ?: LocationEngineProvider.getBestLocationEngine(context)
        freeDriveLocationUpdater = FreeDriveLocationUpdater(locationEngine, locationEngineRequest,
    ...
    private fun initializeContext(context: Context?) {
        if (context == null || context.applicationContext == null) {
            throw IllegalArgumentException(NON_NULL_APPLICATION_CONTEXT_REQUIRED)
        }
    ...
    private fun obtainLocationEngineRequest() = locationEngineRequest
    ...
    private val serviceIntent: Intent
        get() = Intent(applicationContext, NavigationService::class.java)
