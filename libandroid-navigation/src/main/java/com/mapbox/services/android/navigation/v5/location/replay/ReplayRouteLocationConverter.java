...
class ReplayRouteLocationConverter {
    ...
    private static final double factor = 1/10.0;
    ...
    List<Point> sliceRoute(LineString lineString) {
        double distance = calculateDistancePerSec(speed);
        ...
        Point point1 = TurfMeasurement.along(lineString, 0, TurfConstants.UNIT_METERS);
        Point point2 = TurfMeasurement.along(lineString, distance, TurfConstants.UNIT_METERS);

        if (computeCosine(point1, point2) <= cosOf45 ){
            double d = calculateDistancePerSec(speed*0.1);
            for (double i = 0; i < distanceMeters; i += d) {
                Point point = TurfMeasurement.along(lineString, i, TurfConstants.UNIT_METERS);
                points.add(point);
            }
        } else {
            double d = calculateDistancePerSec(speed);
            for (double i = 0; i < distanceMeters; i += d) {
                Point point = TurfMeasurement.along(lineString, i, TurfConstants.UNIT_METERS);
                points.add(point);
            }
    ...
    private double pointOnLine(double slope, double x, double yIntercept) {
     ...
     */
    private double calculateDistancePerSec(double speed) {
        return  (speed * ONE_KM_IN_METERS * delay) / ONE_HOUR_IN_SECONDS;
    }
