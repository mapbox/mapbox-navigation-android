...
internal object NavigationMetricsWrapper {
  ...
  fun arriveEvent(sessionState: SessionState,
                  routeProgress: RouteProgress,
                  location: Location,
                  context: Context) {
    val metricsRouteProgress = MetricsRouteProgress(routeProgress)
    val arriveEvent = NavigationEventFactory
            .buildNavigationArriveEvent(PhoneState(context), sessionState, metricsRouteProgress, location, sdkIdentifier)
    mapboxTelemetry.push(arriveEvent)
  }

  fun cancelEvent(sessionState: SessionState,
                  metricProgress: MetricsRouteProgress,
                  location: Location,
                  context: Context) {
    val cancelEvent = NavigationEventFactory
            .buildNavigationCancelEvent(PhoneState(context), sessionState, metricProgress, location, sdkIdentifier)
    mapboxTelemetry.push(cancelEvent)
  }

  fun departEvent(sessionState: SessionState,
                  metricsRouteProgress: MetricsRouteProgress,
                  location: Location,
                  context: Context) {
    val departEvent = NavigationEventFactory
            .buildNavigationDepartEvent(PhoneState(context), sessionState, metricsRouteProgress, location, sdkIdentifier)
    mapboxTelemetry.push(departEvent)
  }

  fun rerouteEvent(rerouteEvent: RerouteEvent,
                   metricProgress: MetricsRouteProgress,
                   location: Location,
                   context: Context) {
  ...
  fun feedbackEvent(sessionState: SessionState,
                    metricProgress: MetricsRouteProgress,
                    location: Location,
                    description: String,
                    feedbackType: String,
    ...
    val feedbackEvent = NavigationEventFactory.buildNavigationFeedbackEvent(
            PhoneState(context), sessionState, metricProgress, location, sdkIdentifier, description, feedbackType,
            screenshot, feedbackSource)
    mapboxTelemetry.push(feedbackEvent)
  }

  fun routeRetrievalEvent(elapsedTime: Double,
                          routeUuid: String,
                          sessionId: String,
  ...
  fun sendInitialGpsEvent(elapsedTime: Double,
                          sessionId: String) {
