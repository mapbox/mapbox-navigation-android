...
import com.mapbox.navigation.ui.components.databinding.MapboxSpeedInfoViewBinding
...
import com.mapbox.navigation.ui.components.speedlimit.model.ViewConstraints
...
class MapboxSpeedInfoView : FrameLayout {
    ...
    fun render(speedInfo: SpeedInfoValue) {
        ...
        val postedSpeed = speedInfo.postedSpeed
        if (postedSpeed != null) {
            showMutcdOrVienna(speedInfo.speedSignConvention)
            speedInfoPostedSpeedMutcd.text = postedSpeed.toString()
            speedInfoCurrentSpeedMutcd.text = speedInfo.currentSpeed.toString()
            speedInfoCurrentSpeedMutcd.isVisible =
                speedInfo.currentSpeed > postedSpeed && postedSpeed > 0

            speedInfoPostedSpeedVienna.text = postedSpeed.toString()
    ...
    /**
     * Apply runtime changes to [MapboxSpeedInfoView] using [MapboxSpeedInfoOptions]
