...
class RouteNumericTrafficUpdateActivity : AppCompatActivity() {
    ...
    private fun getRoute(): DirectionsRoute {
        val routeAsString = Utils.readRawFileText(this, R.raw.basic_route1)
    ...
    private fun toggleTrafficCongestion() {
        ...
        scope.launch(Dispatchers.Main) {
            while (true) {
                delay(1000)
                var updatedRouteJson = routeAsString
                while (updatedRouteJson.contains("null")) {
                    val newCongestion = (0..100).random().toString()
                    updatedRouteJson = updatedRouteJson.replaceFirst("null", newCongestion)
                }
