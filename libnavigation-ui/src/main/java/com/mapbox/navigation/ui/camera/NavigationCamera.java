...
public class NavigationCamera implements LifecycleObserver {
   ...
   * <p>
   * From the {@link Route}, an initial bearing and target position are created.
   ...
   */
  @NonNull
  private RouteInformation buildRouteInformationFromRoute(Route route) {
  ...
  private long getZoomAnimationDuration(double zoom) {
    ...
    return (long) MathUtils.clamp(
      500 * zoomDiff,
      NavigationConstants.NAVIGATION_MIN_CAMERA_ZOOM_ADJUSTMENT_ANIMATION_DURATION,
  ...
  private long getTiltAnimationDuration(double tilt) {
    ...
    return (long) MathUtils.clamp(
      500 * tiltDiff,
      NavigationConstants.NAVIGATION_MIN_CAMERA_TILT_ADJUSTMENT_ANIMATION_DURATION,
      NavigationConstants.NAVIGATION_MAX_CAMERA_ADJUSTMENT_ANIMATION_DURATION);
  }
