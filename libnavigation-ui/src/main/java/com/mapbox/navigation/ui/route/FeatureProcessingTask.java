...
class FeatureProcessingTask extends Thread {
  ...
  FeatureProcessingTask(List<Route> routes, OnRouteFeaturesProcessedCallback callback, Handler handler) {
    this.routes = routes;
  ...
  private FeatureCollection createRouteFeatureCollection(Route route, boolean isPrimary) {
    ...
    routeFeature.addBooleanProperty(RouteConstants.PRIMARY_ROUTE_PROPERTY_KEY, isPrimary);
  ...
  private List<Feature> buildCongestionFeaturesFromRoute(Route route, LineString lineString,
                                                         boolean isPrimary) {
    final List<Feature> features = new ArrayList<>();
      ...
      if (leg.annotation() != null && leg.annotation().congestion() != null) {
        for (int i = 0; i < leg.annotation().congestion().size(); i++) {
          ...
          if (leg.annotation().congestion().size() + 1 <= lineString.coordinates().size()) {
            ...
            feature.addBooleanProperty(RouteConstants.PRIMARY_ROUTE_PROPERTY_KEY, isPrimary);
