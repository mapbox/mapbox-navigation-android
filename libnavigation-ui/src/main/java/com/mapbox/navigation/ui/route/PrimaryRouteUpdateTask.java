...
class PrimaryRouteUpdateTask extends Thread {
  ...
  public void run() {
    ...
    for (Feature feature : primaryFeatures) {
      ...
      feature.addBooleanProperty(RouteConstants.PRIMARY_ROUTE_PROPERTY_KEY, true);
    }
    ...
    for (FeatureCollection nonPrimaryCollection : updatedRouteCollections) {
      ...
      for (Feature feature : nonPrimaryFeatures) {
        ...
        feature.addBooleanProperty(RouteConstants.PRIMARY_ROUTE_PROPERTY_KEY, false);
      }
    }
    if (cancelThread.get()) {
      return;
    }
    updatedRouteCollections.add(RouteConstants.FIRST_COLLECTION_INDEX, primaryCollection);
    if (!cancelThread.get()) {
