 ...
 * following the given {@link Route} once the UI is initialized.
 * </p>
 */
public class NavigationLauncher {

  /**
   * Starts the UI with a {@link Route} already retrieved from
   * {@link com.mapbox.services.android.navigation.v5.navigation.NavigationRoute}
   *
   ...
   * it back to a {@link Route} object with {@link Gson}.
   *
   * @param context to retrieve {@link SharedPreferences}
   * @return {@link Route} stored when launching
   */
  static Route extractRoute(Context context) {
    SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
    String directionsRouteJson = preferences.getString(NavigationConstants.NAVIGATION_VIEW_ROUTE_KEY, "");
    return Route.fromJson(directionsRouteJson);
  }
