 ...
 * it was launched with a {@link Route}.
 * <p>
 * Or, if not found, this view will look for a set of {@link Point} coordinates.
 * In the latter case, a new {@link Route} will be retrieved from {@link NavigationRoute}.
 * <p>
...
public class NavigationView extends CoordinatorLayout implements LifecycleOwner, OnMapReadyCallback,
   ...
   * for the first time.
   * <p>
   * Zooms to the beginning of the {@link Route}.
   *
   * @param route where camera should move to
   */
  @Override
  public void startCamera(Route route) {
    if (navigationMap != null) {
      navigationMap.startCamera(route);
    }
  }
  ...
  private String establishLanguage(NavigationViewOptions options) {
    String voiceLanguage = options.route().getVoiceLanguage();
  ...
  private String establishUnitType(NavigationViewOptions options) {
    RouteOptionsNavigation routeOptions = options.route().getRouteOptions();
