...
class MapRouteLine {
  ...
  private final HashMap<LineString, Route> routeLineStrings = new HashMap<>();
  private final List<FeatureCollection> routeFeatureCollections = new ArrayList<>();
  private final List<Route> directionsRoutes = new ArrayList<>();
  ...
  MapRouteLine(Context context,
    ...
    wayPointSource = sourceProvider.build(RouteConstants.WAYPOINT_SOURCE_ID, drawnWaypointsFeatureCollection, wayPointGeoJsonOptions);
    ...
    routeLineSource = sourceProvider.build(RouteConstants.ROUTE_SOURCE_ID, drawnRouteFeatureCollection, routeLineGeoJsonOptions);
  ...
  void draw(List<Route> directionsRoutes) {
  ...
  HashMap<LineString, Route> retrieveRouteLineStrings() {
  ...
  List<Route> retrieveDirectionsRoutes() {
  ...
  private void generateRouteFeatureCollectionsFrom(List<Route> routes) {
  ...
  private FeatureProcessingTask retrieveFeatureProcessingTask(List<Route> routes) {
  ...
  private void drawWayPoints() {
    Route primaryRoute = directionsRoutes.get(primaryRouteIndex);
    setWaypointsSource(buildWayPointFeatureCollectionFrom(primaryRoute));
  }

  private FeatureCollection buildWayPointFeatureCollectionFrom(Route route) {
    final List<Feature> wayPointFeatures = new ArrayList<>();
    for (RouteLegNavigation leg : route.getLegs()) {
  ...
  private Feature buildWayPointFeatureFromLeg(RouteLegNavigation leg, int index) {
    ...
    feature.addStringProperty(RouteConstants.WAYPOINT_PROPERTY_KEY, index == 0 ? RouteConstants.WAYPOINT_ORIGIN_VALUE : RouteConstants.WAYPOINT_DESTINATION_VALUE);
    return feature;
  }
  ...
  private void updateAlternativeVisibilityTo(boolean isAlternativeVisible) {
    ...
    for (Layer layer : routeLayers) {
      ...
      if (layerId.equals(RouteConstants.ROUTE_LAYER_ID) || layerId.equals(RouteConstants.ROUTE_SHIELD_LAYER_ID)) {
        LineLayer route = (LineLayer) layer;
        if (isAlternativeVisible) {
          route.setFilter(literal(true));
        } else {
          route.setFilter(Expression.eq(Expression.get(RouteConstants.PRIMARY_ROUTE_PROPERTY_KEY), true));
        }
      }
    }
  }
