...
public class NavigationMapboxMap {
  ...
  /**
   * Will draw the given {@link Route} on the map using the colors defined
   * in your given style.
   *
   * @param route to be drawn
   */
  public void drawRoute(@NonNull Route route) {
    mapRoute.addRoute(route);
  }

  /**
   * Will draw the given list of {@link Route} on the map using the colors defined
   * in your given style.
   * <p>
   ...
   */
  public void drawRoutes(@NonNull List<Route> routes) {
  ...
  /**
   * Centers the map camera to the beginning of the provided {@link Route}.
   *
   * @param route to update the camera position
   */
  public void startCamera(@NonNull Route route) {
    mapCamera.start(route);
  }
  ...
  /**
   * Adjusts the map camera to {@link Route} being traveled along.
   * <p>
  ...
  private void initializeLocationComponent(MapView mapView, MapboxMap map) {
    ...
    map.setMinZoomPreference(NavigationConstants.NAVIGATION_MINIMUM_MAP_ZOOM);
