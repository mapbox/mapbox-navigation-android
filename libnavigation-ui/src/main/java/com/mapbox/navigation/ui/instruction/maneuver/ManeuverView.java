...
public class ManeuverView extends View {
  ...
  private static final Set<String> SHOULD_FLIP_MODIFIERS = new HashSet<String>() {
    {
      add(NavigationConstants.STEP_MANEUVER_MODIFIER_SLIGHT_LEFT);
      add(NavigationConstants.STEP_MANEUVER_MODIFIER_LEFT);
      add(NavigationConstants.STEP_MANEUVER_MODIFIER_SHARP_LEFT);
      add(NavigationConstants.STEP_MANEUVER_MODIFIER_UTURN);
    }
  };
  ...
  private PointF size;
  private String drivingSide = NavigationConstants.STEP_MANEUVER_MODIFIER_RIGHT;
  ...
  public void setDrivingSide(String drivingSide) {
    if (NavigationConstants.STEP_MANEUVER_MODIFIER_LEFT.equals(drivingSide) || NavigationConstants.STEP_MANEUVER_MODIFIER_RIGHT.equals(drivingSide)) {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (ROUNDABOUT_MANEUVER_TYPES.contains(maneuverType)) {
      flip = NavigationConstants.STEP_MANEUVER_MODIFIER_LEFT.equals(drivingSide);
    }
    if (NavigationConstants.STEP_MANEUVER_MODIFIER_LEFT.equals(drivingSide) && NavigationConstants.STEP_MANEUVER_MODIFIER_UTURN.contains(maneuverModifier)) {
      setScaleX(flip ? 1 : -1);
    } else {
      setScaleX(flip ? -1 : 1);
    }
  ...
  private String checkManeuverModifier(String maneuverType, String maneuverModifier) {
    if (!maneuverType.contentEquals(NavigationConstants.STEP_MANEUVER_TYPE_ARRIVE) && maneuverModifier != null) {
      maneuverType = null;
    }
    return maneuverType;
  }
