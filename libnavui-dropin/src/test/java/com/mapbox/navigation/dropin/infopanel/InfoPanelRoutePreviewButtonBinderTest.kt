...
class InfoPanelRoutePreviewButtonBinderTest {
    ...
    private lateinit var sut: InfoPanelRoutePreviewButtonBinder
    ...
    fun setUp() {
        ...
        sut = InfoPanelRoutePreviewButtonBinder(navContext)
    }

    @Test
    fun `bind should bind RoutePreviewButtonComponent`() {
    ...
    fun `bind should reload RoutePreviewButtonComponent when routePreviewButtonStyle changes`() =
        runBlockingTest {
            val components = sut.bind(viewGroup)
            components.onAttached(mapboxNavigation)

            val firstComponent = components.findComponent { it is RoutePreviewButtonComponent }
            navContext.applyStyleCustomization {
                routePreviewButtonStyle = R.style.Widget_AppCompat_Button
            }
            val secondComponent = components.findComponent { it is RoutePreviewButtonComponent }
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `bind should re-create MapboxExtendableButton when routePreviewButtonStyle changes`() =
        runBlockingTest {
            val components = sut.bind(viewGroup)
            components.onAttached(mapboxNavigation)
