...
class RecenterButtonBinderTest {
    ...
    fun `bind should reload RecenterButtonComponent when style recenterButtonParams changes`() =
        runBlockingTest {
            val components = sut.bind(viewGroup)
            components.onAttached(mapboxNavigation)
            ...
            navContext.applyStyleCustomization {
                recenterButtonParams = buttonParams
            }
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `bind should re-create MapboxExtendableButton when style recenterButtonParams changes`() =
        runBlockingTest {
            val components = sut.bind(viewGroup)
            components.onAttached(mapboxNavigation)
