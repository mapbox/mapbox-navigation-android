...
class NavigationCameraTest {
    ...
    private val stateTransition: NavigationCameraStateTransition = mockk(relaxUnitFun = true) {
        every { transitionToFollowing(any()) } returns followingAnimatorSet
        every { transitionToOverview(any()) } returns overviewAnimatorSet
        every { updateFrameForFollowing(any()) } returns followingFrameAnimatorSet
        every { updateFrameForOverview(any()) } returns overviewFrameAnimatorSet
    }
    ...
    fun `when following requested, listener notified`() {
        navigationCamera.requestNavigationCameraToFollowing(animatorListener)
    ...
    fun `when overview requested, listener notified`() {
        navigationCamera.requestNavigationCameraToOverview(animatorListener)
    ...
    fun `when following frame ends, unregister animators`() {
        ...
        internalTransitionListenerSlot.captured.onAnimationEnd(followingAnimatorSet)
        internalDataSourceObserverSlot.captured.viewportDataSourceUpdated(viewportData)
        internalFrameListenerSlot.captured.onAnimationStart(followingFrameAnimatorSet)
        internalFrameListenerSlot.captured.onAnimationEnd(followingFrameAnimatorSet)

        followingFrameAnimatorSet.childAnimations.forEach {
            verify(exactly = 1) { cameraPlugin.unregisterAnimators(it as ValueAnimator) }
        }
    }
    ...
    fun `when overview frame ends, unregister animators`() {
        ...
        internalTransitionListenerSlot.captured.onAnimationEnd(overviewAnimatorSet)
        internalDataSourceObserverSlot.captured.viewportDataSourceUpdated(viewportData)
        internalFrameListenerSlot.captured.onAnimationStart(overviewFrameAnimatorSet)
        internalFrameListenerSlot.captured.onAnimationEnd(overviewFrameAnimatorSet)

        overviewFrameAnimatorSet.childAnimations.forEach {
            verify(exactly = 1) { cameraPlugin.unregisterAnimators(it as ValueAnimator) }
        }
    }
    ...
    fun `when following and idle requested, cancel animations and change state`() {
        ...
        navigationCamera.requestNavigationCameraToIdle()
        internalFrameListenerSlot.captured.onAnimationCancel(followingFrameAnimatorSet)
        internalFrameListenerSlot.captured.onAnimationEnd(followingFrameAnimatorSet)

        verify(exactly = 1) { followingFrameAnimatorSet.cancel() }
        assertEquals(NavigationCameraState.IDLE, navigationCamera.state)
    }
    ...
    fun `when overview and idle requested, cancel animations and change state`() {
        ...
        navigationCamera.requestNavigationCameraToIdle()
        internalFrameListenerSlot.captured.onAnimationCancel(overviewFrameAnimatorSet)
        internalFrameListenerSlot.captured.onAnimationEnd(overviewFrameAnimatorSet)

        verify(exactly = 1) { overviewFrameAnimatorSet.cancel() }
        assertEquals(NavigationCameraState.IDLE, navigationCamera.state)
    }
    ...
    private fun verifyTransitionExecuted(
        transitionFun: NavigationCameraStateTransition.(CameraOptions) -> AnimatorSet,
        transitionCameraOptions: CameraOptions,
        returnTransitionSet: AnimatorSet,
        instant: Boolean = false,
        times: Int = 1
    ) {
        verify(exactly = times) { stateTransition.transitionFun(transitionCameraOptions) }
