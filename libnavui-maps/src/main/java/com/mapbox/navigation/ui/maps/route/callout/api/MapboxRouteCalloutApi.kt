...
class MapboxRouteCalloutApi(
    ...
    private val alternativesGeometryDifference = mutableMapOf<String, LineString>()
    ...
    fun setNavigationRoutes(
        ...
        routes.firstOrNull()?.run {
            ...
            val alternativesGeometryDifference = alternativeRoutes.associate { altRoute ->
                altRoute.id to extractDifferentGeometry(
                    primaryRoutePoints,
                    altRoute,
                    distinctAlternativeRouteMetadata.firstOrNull {
                        it.navigationRoute.id == altRoute.id
                    },
                )
            }

            this@MapboxRouteCalloutApi.alternativesGeometryDifference.clear()
            this@MapboxRouteCalloutApi.alternativesGeometryDifference.putAll(
                alternativesGeometryDifference,
            )
        }
    ...
    private fun createActiveGuidanceRouteCallouts(
        ...
        return alternativeRoutes.map { route ->
            ...
            val geometry = alternativesGeometryDifference[route.id]
                ?: route.directionsRoute.completeGeometryToLineString()
