 ...
 *
 * This object is only a wrapper of [MapboxNavigationTelemetry] to cache/retrieve/post navigation.feedback event.
 * It must be used after [MapboxNavigationTelemetry.initialize] has been called.
 *
 * A complete feedback cache flow will be:
 * 1. [cacheUserFeedback] to generate navigation.feedback event in [MapboxNavigationTelemetry] internally.
 * 2. [getCachedUserFeedback] to retrieve all cached navigation.feedback events from [MapboxNavigationTelemetry].
 * 3. make possible changes to the list that returned by [getCachedUserFeedback].
