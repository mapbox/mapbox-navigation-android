...
class ChargingStationsMetadataUpdaterTest {
            ...
            RouteOptions.fromUrl(URL(it)).let {
                it.toBuilder()
                    .unrecognizedJsonProperties(
                        it.unrecognizedJsonProperties?.apply {
    ...
    @Test
    fun `offline EV route has waypoints per response`() = runBlocking {
        val onlineRoute = createTestBerlinOnlineEvRouteAfterReroute().first()
        val offlineRoutes = createTestBerlinOfflineEvRouteAfterRerouteWithWaypointsPerResponse()
        val result = restoreChargingStationsMetadata(onlineRoute, offlineRoutes)
        result.forEach { navigationRoute ->
            assertEquals(
                listOf(null, "ocm-54453", null),
                navigationRoute.getChargingStationIds()
            )
    ...
    private fun createTestBerlinOfflineEvRouteAfterReroute(
        ...
        val url = "http://localhost:57725/directions/v5/mapbox/driving-traffic/" +
            "13.3674044,52.4974382;13.377746,52.51224;13.393451,52.5091392" +
            "?access_token=***&geometries=polyline6&alternatives=true&overview=full" +
            "&steps=true&avoid_maneuver_radius=40.0&bearings=280%2C90%3B%3B" +
            "&layers=%3B%3B&continue_straight=true" +
            "&annotations=state_of_charge&roundabout_exits=true" +
            "&voice_instructions=true&banner_instructions=true" +
            "&enable_refresh=true" +
            "&snapping_include_closures=true%3B%3B&snapping_include_static_closures=true%3B%3B" +
            "&waypoints_per_route=true&waypoints.charging_station_power=%3B50000%3B" +
            "&ev_max_charge=50000" +
            "&ev_charging_curve=0%2C100000%3B40000%2C70000%3B60000%2C30000%3B80000%2C10000" +
            "&engine=electric&energy_consumption_curve=0%2C300%3B20%2C160%3B80%2C140%3B120%2C180" +
            "&ev_min_charge_at_charging_station=1" +
            "&waypoints.charging_station_id=%3Bocm-54453%3B" +
            "&waypoints.charging_station_current_type=%3Bdc%3B&ev_add_charging_stops=false" +
            "&ev_initial_charge=1000&ev_connector_types=ccs_combo_type1%2Cccs_combo_type2"
    ...
    private fun createTestBerlinOfflineEvRouteAfterRerouteWithWaypointsPerResponse():
        List<NavigationRoute> {
        val url =
            "http://localhost:57725/directions/v5/mapbox/driving-traffic/" +
                "13.3674044,52.4974382;13.377746,52.51224;13.393451,52.5091392?access_token=***" +
                "&geometries=polyline6&alternatives=false&overview=full&steps=true" +
                "&avoid_maneuver_radius=40.0&bearings=280%2C90%3B%3B&layers=%3B%3B" +
                "&continue_straight=true&annotations=state_of_charge&roundabout_exits=true" +
                "&voice_instructions=true&banner_instructions=true&enable_refresh=true" +
                "&snapping_include_closures=true%3B%3B" +
                "&snapping_include_static_closures=true%3B%3B" +
                "&waypoints_per_route=false&waypoints.charging_station_power=%3B50000%3B" +
                "&ev_max_charge=50000" +
                "&ev_charging_curve=0%2C100000%3B40000%2C70000%3B60000%2C30000%3B80000%2C10000" +
                "&engine=electric" +
                ...
                "&ev_initial_charge=1000&ev_connector_types=ccs_combo_type1%2Cccs_combo_type2"
        return NavigationRoute.create(
            resourceAsString("berlinOfflineEvRouteAfterRerouteWaypointsPerResponse.json"),
            url,
            RouterOrigin.Onboard
        )
    }
