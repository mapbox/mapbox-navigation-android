...
fun createContext(packageName: String): Context {
    ...
    every { mockedContext.mainLooper } returns mockk(relaxed = true)
    ...
    every { mockedContext.applicationContext.packageManager } returns mockk(relaxed = true)
//    every { mockedContext.packageManager } returns mockk(relaxed = true)
    every { mockedContext.applicationContext.packageName } returns packageName
//    every { mockedContext.packageManager.getApplicationInfo(mockedContext.packageName, PackageManager.GET_META_DATA) } returns mockk(relaxed = true)
