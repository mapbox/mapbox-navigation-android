...
import com.mapbox.navigation.voice.api.VoiceInstructionsFilePlayerProvider.retrieveVoiceInstructionsFilePlayer
import com.mapbox.navigation.voice.api.VoiceInstructionsTextPlayerProvider.retrieveVoiceInstructionsTextPlayer
...
class MapboxVoiceInstructionsPlayerTest {
    ...
    fun shutdown() {
        ...
        val afterShotDownConsumer = mockk<MapboxNavigationConsumer<SpeechAnnouncement>>(
            relaxed = true,
        )
        val afterShutdownAnnouncement = SpeechAnnouncement.Builder("shutdown")
            .build()
        mapboxVoiceInstructionsPlayer.play(
            afterShutdownAnnouncement,
            afterShotDownConsumer,
        )
        verify(exactly = 0) { mockedFilePlayer.play(any(), any()) }
        verify(exactly = 0) { mockedTextPlayer.play(any(), any()) }
        verify(exactly = 0) { afterShotDownConsumer.accept(any()) }
    }
