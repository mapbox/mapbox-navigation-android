...
import com.mapbox.navigation.voice.api.AudioFocusDelegateProvider.defaultAudioFocusDelegate
...
class MapboxVoiceInstructionsPlayer @JvmOverloads constructor(
    ...
    fun play(
        ...
        if (isShutDown) {
            logE(TAG) {
                "play is called after MapboxVoiceInstructionsPlayer was shutdown. " +
                    "Invocation is ignored, consumer callback is not called. " +
    ...
    private fun play() {
        ...
        logD(TAG) {
            "Peeked from the playCallbackQueue ${currentPlayCallback.announcement}"
        }
        ...
        requestFocus(owner) { isGranted ->
            logD(TAG) {
                "Request focus callback: isGranted=$isGranted, owner=$owner"
            }
            if (isGranted) {
                when (owner) {
    ...
    private companion object {
        ...
        private const val TAG = "MapboxVoiceInstructionsPlayer"
